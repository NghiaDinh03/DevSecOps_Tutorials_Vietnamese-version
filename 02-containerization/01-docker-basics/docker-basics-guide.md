...
### 🛡️ Nguyên lý 4: Bảo mật Ứng dụng Tích hợp AI/LLM (AI & LLM App Security for DevOps)
Trong kỷ nguyên trí tuệ nhân tạo, các ứng dụng tích hợp mô hình ngôn ngữ lớn (như chatbot sử dụng mô hình Ollama hoặc kết nối qua API của OpenAI) rất dễ trở thành mục tiêu của các kiểu tấn công mới. Khi đóng gói ứng dụng AI bằng Docker, bạn cần chú ý:
*   **Cấm lưu trữ cứng API Keys (như `OPENAI_API_KEY`) trong biến môi trường `ENV` của Dockerfile:** Hãy nạp động qua biến môi trường khi chạy container hoặc qua cơ chế Vault.
*   **Phòng chống rò rỉ dữ liệu nhạy cảm qua Prompt:** Triển khai cơ chế kiểm duyệt đầu vào (Input Sanitization) và kiểm duyệt đầu ra (Output Guardrails) để ngăn ngừa lỗ hổng **Prompt Injection** (thao túng mô hình AI phá vỡ các rào cản nghiệp vụ của hệ thống).
*   **Quét an toàn thư viện Python AI định kỳ:** Các thư viện AI của Python (như `langchain`, `transformers`, `torch`) thường có dung lượng rất lớn và tần suất xuất hiện các lỗ hổng bảo mật nghiêm trọng (CVEs) rất cao. Luôn dùng Trivy để quét ảnh container chứa AI app trước khi phân phối.
...
5. *Lỗ hổng Prompt Injection hoạt động thế nào trong ứng dụng AI và tại sao DevOps cần phải quét lỗ hổng thư viện Python AI bằng Trivy?*
...
### 🇻🇳 [Xây Dựng Dockerfile An Toàn Theo Bộ Tiêu Chuẩn CIS Benchmarks (Hardening Dockerfile using CIS Standards)](./blog/dockerfile-hardening-cis-benchmarks.md)
*   **Chi tiết**: Hướng dẫn 10 bước gia cố Dockerfile nghiêm ngặt theo bộ tiêu chuẩn bảo mật danh tiếng CIS Docker Benchmarks được dịch và biên soạn chuẩn hóa.
*   **Giá trị thực tiễn**: Cung cấp checklist chi tiết từ cấm tag latest, cấu hình HEALTHCHECK, cấm SUID/SGID nguy hiểm đến Dockerfile mẫu hoàn thiện đạt chuẩn.
*   **Liên kết nguồn gốc**: [Snyk Security Blog - Dockerfile Security Best Practices](https://snyk.io/blog/dockerfile-security-best-practices/)
