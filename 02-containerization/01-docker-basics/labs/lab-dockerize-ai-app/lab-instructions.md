# 🧪 Lab 01: Đóng gói & Gia cố Bảo mật Ứng dụng Web Chatbot

## 🎯 Mục tiêu bài thực hành
Sau bài lab này, bạn sẽ tự tay đóng gói một ứng dụng Web Chatbot (viết bằng NodeJS) thành một Docker Image đạt chuẩn an toàn sản xuất (Production-ready).
*   Biết cách thiết lập cấu trúc file loại trừ tài nguyên thừa (`.dockerignore`) tại thư mục gốc của bối cảnh build.
*   Áp dụng kỹ thuật **Multi-stage Build** tối ưu hóa dung lượng image từ ~900MB xuống dưới ~150MB.
*   Gia cố an toàn hệ thống bằng cách chạy container dưới quyền **Non-root User** (UID 10001) thay vì quyền root mặc định.
*   Khởi chạy container ở chế độ bảo mật cao: **Read-only Root Filesystem** kết hợp chặn leo thang đặc quyền.

---

## 💼 Bối cảnh & Mô hình thực hành (Self-contained Lab)
Ứng dụng đích cần đóng gói là một ứng dụng Web Chatbot gọn nhẹ được chuẩn bị sẵn mã nguồn trực tiếp trong thư mục bài lab này, bao gồm:
*   `server.js`: Web API phục vụ chat giả lập.
*   `package.json`: Khai báo thư viện phụ thuộc ExpressJS.
*   `public/index.html`: Giao diện chatbot đẹp mắt để tương tác.

Toàn bộ môi trường thực hành này là hoàn toàn khép kín. Bạn chỉ cần mở terminal tại thư mục bài lab và thực hiện các câu lệnh build tại chỗ mà không cần phải chuẩn bị thêm bất kỳ dự án ngoài nào khác.
...
Để tránh việc sao chép nhầm các file nhạy cảm (như tệp `.env`), các file log rác, hoặc thư mục `node_modules` cũ từ máy host vào trong image (gây xung đột hệ điều hành), chúng ta cần định nghĩa một file loại trừ.

Tệp tin `.dockerignore` đã được đặt sẵn tại thư mục gốc của bài lab này để Docker tự động nhận diện trong bối cảnh build:
...
Hãy cùng phân tích tệp cấu hình Dockerfile mẫu đạt chuẩn an ninh cao được đặt tại thư mục: `configs/Dockerfile.secure`

Các điểm cải tiến bảo mật (Security Hardening) trong Dockerfile này:
1.  **Sử dụng Base Image chính chủ & Tối giản**: `node:20-alpine` giúp loại bỏ các thư viện hệ thống thừa thãi và giảm thiểu bề mặt tấn công.
2.  **Multi-stage Build**: 
    *   *Stage 1 (builder)*: Cài đặt tất cả dependencies và đóng gói ứng dụng.
...
3.  **Tạo tài khoản không đặc quyền (Non-root User)**: 
    ...
    *   Gọi chỉ thị `USER 10001` trước khi chạy tiến trình chính để cấm container breakout.

---

### 📋 Bước 4: Thực hiện Build Image tại chỗ
Di chuyển terminal của bạn vào đúng thư mục bài lab `02-containerization/01-docker-basics/labs/lab-dockerize-ai-app/` và chạy lệnh build tại chỗ:

```bash
docker build -f configs/Dockerfile.secure -t devsecops-gemma-app .
```

> **Giải thích câu lệnh**:
> *   `-f configs/Dockerfile.secure`: Chỉ định rõ file công thức Dockerfile cần build.
> *   `-t devsecops-gemma-app`: Đặt tên nhãn (tag) cho Image sản phẩm.
> *   `.`: Sử dụng chính thư mục hiện tại làm bối cảnh build (Build Context).

Sau khi build hoàn tất, hãy kiểm tra kích thước image sản phẩm so với image node cơ bản:
```bash
docker images | grep devsecops-gemma-app
```
...
Trước khi triển khai Docker Image lên môi trường Production, một kỹ sư DevSecOps bắt buộc phải thực hiện quét lỗ hổng bảo mật để phát hiện các thư viện bị lỗi thời hoặc các lỗi hệ điều hành (CVE). Công cụ quét mã nguồn mở chuẩn công nghiệp hiện nay là **Trivy** của Aqua Security.

Bạn có thể chạy Trivy trực tiếp dưới dạng một container tạm thời để quét image `devsecops-gemma-app` mà không cần cài đặt Trivy lên máy cá nhân của mình.
...
*   **HIGH** & **CRITICAL**: Các lỗ hổng cực kỳ nguy hiểm (ví dụ: lỗi thực thi mã từ xa - RCE). Nếu gặp các lỗi này, pipeline CI/CD thường sẽ bị cấu hình chặn đứng (Fail-fast).
...
Mở trình duyệt truy cập địa chỉ: `http://localhost:3000`. Hệ giao diện chat của ứng dụng chatbot phải hiển thị và hoạt động bình thường. Bạn có thể gõ các câu hỏi về "security" hoặc "bảo mật" để thử nghiệm phản hồi thông minh từ app.
...
```
*Kết quả mong đợi:* Hệ thống trả về User ID `10001` (hoặc báo `unknown uid 10001` do Alpine không phân giải được tên, nhưng hoàn toàn không phải `root`).
