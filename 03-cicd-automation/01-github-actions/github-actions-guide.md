# 🐙 Sub-module 01: GitHub Actions — Cơ Chế Hoạt Động & Kỹ Thuật Gia Cố Bảo Mật Runner (Self-Hosted Runner Hardening)
...
## 5. Lớp Học Vỡ Lòng Về GitOps & ArgoCD (GitOps Basics From Zero)

Trong kiến trúc phân phối phần mềm hiện đại Cloud-Native, mô hình triển khai liên tục (CD) truyền thống đang dần được thay thế bằng mô hình **GitOps**.

### A. Nguyên lý GitOps: Git là nguồn chân lý duy nhất (Git as Single Source of Truth)
Trong mô hình CD truyền thống (Push-based), pipeline CI/CD (như GitHub Actions runner) sẽ trực tiếp chạy lệnh `kubectl apply` hoặc các script deploy để đẩy ứng dụng lên Kubernetes cluster. Điều này yêu cầu runner phải nắm giữ khóa quản trị tối cao `kubeconfig`, mở ra lỗ hổng bảo mật lớn nếu runner bị hack.

Ngược lại, **GitOps** áp dụng cơ chế kéo dữ liệu (**Pull-based**):
*   Toàn bộ cấu hình hạ tầng Kubernetes (YAML manifests, Helm charts) được lưu trữ và quản lý phiên bản trong một Git Repository chuyên dụng (GitOps Repo).
*   Một chương trình chạy ngầm (Agent) bên trong cụm Kubernetes - điển hình là **ArgoCD** - liên tục theo dõi (poll) GitOps Repo này.
*   Khi phát hiện có sự thay đổi trên Git (v.d. lập trình viên merge code cập nhật tag image mới), ArgoCD sẽ tự động kéo cấu hình về và áp dụng lên cluster (**Auto-sync**).
*   Nếu ai đó tự ý sửa đổi thủ công cấu hình trực tiếp trên cluster (v.d. chạy lệnh `kubectl edit`), ArgoCD sẽ lập tức phát hiện sự sai lệch này (**Drift Detection**) và tự động đè lại cấu hình chuẩn từ Git để bảo vệ tính nhất quán hệ thống.

---

## 6. Câu hỏi ôn tập chuyên sâu (Deep-dive Quiz)
...
4.  *Mô tả sự khác biệt cốt lõi và ưu thế bảo mật của mô hình CD kéo (Pull-based GitOps) sử dụng ArgoCD so với mô hình CD đẩy (Push-based) truyền thống?*

---

## 7. Tài nguyên Đọc thêm Chất lượng cao (Recommended Blog Readings)

### 🇬🇧 [Securing GitHub Actions: How to Avoid OWASP Top 10 CI/CD Security Risks (Bảo Mật GitHub Actions: Cách Tránh 10 Rủi Ro Bảo Mật CI/CD Hàng Đầu Theo OWASP)](./blog/securing-github-actions.md)
*   **Chi tiết**: Bản dịch thuật & tóm tắt chuyên sâu 100% tiếng Việt của bài nghiên cứu bảo mật hệ thống CI/CD từ Cider Security được lưu trữ cục bộ.
*   **Giá trị thực tiễn**: Nhận diện các vector tấn công hàng đầu như đầu độc pipeline (*PPE*), đặc quyền token dư thừa và các biện pháp bảo vệ Self-hosted Runner.
*   **Liên kết nguồn gốc**: [Dev.to - Securing GitHub Actions](https://dev.to/cidersecurity/securing-github-actions-how-to-avoid-owasp-top-10-cicd-security-risks-2b8)

### 🇻🇳 [Bảo Mật Hệ Thống GitOps Với ArgoCD: Các Thực Tiễn Hardening Tốt Nhất (Hardening ArgoCD in GitOps Pipelines)](./blog/securing-gitops-argocd.md)
*   **Chi tiết**: Hướng dẫn gia cố ArgoCD chuẩn doanh nghiệp được dịch sát nghĩa từ tài liệu bảo mật chính thống của dự án Argo Project.
*   **Giá trị thực tiễn**: Các kỹ thuật tắt tài khoản admin mặc định, phân tách quyền bằng AppProjects, thiết lập Network Policies cô lập repo-server và bảo mật secrets GitOps.
*   **Liên kết nguồn gốc**: [Argo Project Security Guidelines](https://argo-cd.readthedocs.io/en/stable/operator-manual/security/)
